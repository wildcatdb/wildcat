...
func TestTrimUnusedBlocks(t *testing.T) {
	// Create a path for a new file
	tempFilePath := os.TempDir() + "/blockmanager_trim_test"
	...
	// Append some data blocks
	data1 := []byte("First block of data")
	data2 := []byte("Second block of data")
	data3 := []byte("Third block of data")
	...
	// Get file size after appending data
	fileInfo, err = bm.file.Stat()
	if err != nil {
		t.Fatalf("Failed to get file info after appends: %v", err)
	}
	sizeAfterAppends := fileInfo.Size()

	// Count free blocks before trim
	freeBlocksBeforeTrim := 0
	tempQueue := queue.New()
	for !bm.allocationTable.IsEmpty() {
		blockID := bm.allocationTable.Dequeue()
		tempQueue.Enqueue(blockID)
		freeBlocksBeforeTrim++
	}
	bm.allocationTable = tempQueue

	t.Logf("Free blocks before trim: %d", freeBlocksBeforeTrim)

	// Trim unused blocks
	blocksTrimmed, err := bm.TrimUnusedBlocks()
	if err != nil {
		t.Fatalf("Failed to trim unused blocks: %v", err)
	}

	t.Logf("Blocks trimmed: %d", blocksTrimmed)

	// Get file size after trim
	fileInfo, err = bm.file.Stat()
	if err != nil {
		t.Fatalf("Failed to get file info after trim: %v", err)
	}
	sizeAfterTrim := fileInfo.Size()

	// File should be smaller after trim
	if sizeAfterTrim >= sizeAfterAppends {
		t.Errorf("File size didn't decrease after trim. Before: %d, After: %d",
			sizeAfterAppends, sizeAfterTrim)
	}

	// Count free blocks after trim
	freeBlocksAfterTrim := 0
	tempQueue = queue.New()
	for !bm.allocationTable.IsEmpty() {
		blockID := bm.allocationTable.Dequeue()
		tempQueue.Enqueue(blockID)
	...
	t.Logf("Free blocks after trim: %d", freeBlocksAfterTrim)

	// Should have fewer free blocks after trim
	if freeBlocksAfterTrim >= freeBlocksBeforeTrim {
		t.Errorf("Free blocks didn't decrease after trim. Before: %d, After: %d",
			freeBlocksBeforeTrim, freeBlocksAfterTrim)
	}

	// Verify we can still read all our data
	readData1, _, err := bm.Read(blockID1)
	if err != nil {
		t.Fatalf("Failed to read data1 after trim: %v", err)
	}
	if !bytes.Equal(data1, readData1) {
		t.Errorf("Data1 mismatch after trim. Expected: %s, Got: %s",
			string(data1), string(readData1))
	}

	readData2, _, err := bm.Read(blockID2)
	if err != nil {
		t.Fatalf("Failed to read data2 after trim: %v", err)
	}
	if !bytes.Equal(data2, readData2) {
		t.Errorf("Data2 mismatch after trim. Expected: %s, Got: %s",
			string(data2), string(readData2))
	}

	readData3, _, err := bm.Read(blockID3)
	if err != nil {
		t.Fatalf("Failed to read data3 after trim: %v", err)
	}
	if !bytes.Equal(data3, readData3) {
		t.Errorf("Data3 mismatch after trim. Expected: %s, Got: %s",
			string(data3), string(readData3))
	}

	// Test trimming a file with no unused blocks at the end
	blocksTrimmedAgain, err := bm.TrimUnusedBlocks()
	if err != nil {
		t.Fatalf("Failed to trim again: %v", err)
	}

	if blocksTrimmedAgain != 0 {
		t.Errorf("Expected 0 blocks trimmed on second trim, got %d", blocksTrimmedAgain)
	}

	// Verify file size didn't change
	fileInfo, err = bm.file.Stat()
	if err != nil {
		t.Fatalf("Failed to get file info after second trim: %v", err)
	}
	sizeAfterSecondTrim := fileInfo.Size()

	if sizeAfterSecondTrim != sizeAfterTrim {
		t.Errorf("File size changed after second trim. Expected: %d, Got: %d",
			sizeAfterTrim, sizeAfterSecondTrim)
	}
}

func TestTrimUnusedBlocksWithLargeFile(t *testing.T) {
	// Create a path for a new file
	tempFilePath := os.TempDir() + "/blockmanager_trim_large_test"
	...
	// Append data only in the first few blocks
	numDataBlocks := 5
	blockIDs := make([]int64, numDataBlocks)

	for i := 0; i < numDataBlocks; i++ {
		data := []byte(fmt.Sprintf("Data block %d", i))
		blockID, err := bm.Append(data)
		if err != nil {
			t.Fatalf("Failed to append data block %d: %v", i, err)
		}
	...
	// Force allocation of many more blocks at the end
	for i := 0; i < 10; i++ { // This will allocate 10 * Allotment blocks
		if err := bm.appendFreeBlocks(); err != nil {
			t.Fatalf("Failed to append free blocks: %v", err)
		}
	}

	// Get file size before trim
	fileInfo, err := bm.file.Stat()
	if err != nil {
		t.Fatalf("Failed to get file info: %v", err)
	}
	sizeBeforeTrim := fileInfo.Size()
	...
	t.Logf("Trimmed %d blocks from large file", blocksTrimmed)

	// Should have trimmed a significant number of blocks
	if blocksTrimmed < uint64(10*Allotment-10) { // Approximate, since some blocks were used
		t.Errorf("Expected to trim many blocks, only trimmed %d", blocksTrimmed)
	}
	...
	sizeReduction := sizeBeforeTrim - sizeAfterTrim
	expectedReduction := int64(blocksTrimmed) * int64(BlockSize)

	if sizeReduction != expectedReduction {
		t.Errorf("Size reduction mismatch. Expected: %d, Got: %d",
			expectedReduction, sizeReduction)
	}

	t.Logf("File size reduced from %d to %d bytes (reduction: %d bytes)",
		sizeBeforeTrim, sizeAfterTrim, sizeReduction)

	// Verify all original data is still readable
	for i, blockID := range blockIDs {
		expectedData := []byte(fmt.Sprintf("Data block %d", i))
		readData, _, err := bm.Read(blockID)
		if err != nil {
			t.Fatalf("Failed to read block %d after trim: %v", i, err)
		}
		if !bytes.Equal(expectedData, readData) {
			t.Errorf("Data mismatch for block %d after trim", i)
		}
	}
}

func TestTrimUnusedBlocksEmptyFile(t *testing.T) {
	// Create a path for a new file
	tempFilePath := os.TempDir() + "/blockmanager_trim_empty_test"
	...
	// Trim without adding any data
	blocksTrimmed, err := bm.TrimUnusedBlocks()
	if err != nil {
		t.Fatalf("Failed to trim unused blocks: %v", err)
	}

	// Should trim most blocks except keeping at least one
	if blocksTrimmed == 0 {
		t.Errorf("Expected to trim some blocks from empty file, got 0")
	}
	...
	if sizeAfterTrim >= initialSize {
		t.Errorf("File size didn't decrease after trim. Before: %d, After: %d",
			initialSize, sizeAfterTrim)
	}

	// Should still be able to append data
	testData := []byte("Test data after trim")
	blockID, err := bm.Append(testData)
	if err != nil {
		t.Fatalf("Failed to append data after trim: %v", err)
	}

	// Verify we can read it back
	readData, _, err := bm.Read(blockID)
	if err != nil {
		t.Fatalf("Failed to read data after trim: %v", err)
	}

	if !bytes.Equal(testData, readData) {
		t.Errorf("Data mismatch after trim. Expected: %s, Got: %s",
